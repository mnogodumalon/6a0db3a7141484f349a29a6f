...
import type { Anmerkungen, CreateAnmerkungen } from '@/types/app';
...
export class LivingAppsService {
  // --- ANMERKUNGEN ---
  static async getAnmerkungen(): Promise<Anmerkungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANMERKUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Anmerkungen[];
    return enrichLookupFields(records, 'anmerkungen');
  }
  static async getAnmerkungenEntry(id: string): Promise<Anmerkungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ANMERKUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Anmerkungen;
    return enrichLookupFields([record], 'anmerkungen')[0];
  }
  static async createAnmerkungenEntry(fields: CreateAnmerkungen) {
    return callApi('POST', `/apps/${APP_IDS.ANMERKUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'anmerkungen') });
  }
  static async updateAnmerkungenEntry(id: string, fields: Partial<CreateAnmerkungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.ANMERKUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'anmerkungen') });
  }
  static async deleteAnmerkungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ANMERKUNGEN}/records/${id}`);
  }
