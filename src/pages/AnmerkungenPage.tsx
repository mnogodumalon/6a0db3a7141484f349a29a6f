...
export default function AnmerkungenPage() {
  const [records, setRecords] = useState<Anmerkungen[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Anmerkungen | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Anmerkungen | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Anmerkungen | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getAnmerkungen());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Anmerkungen['fields']) {
    await LivingAppsService.createAnmerkungenEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Anmerkungen['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateAnmerkungenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Anmerkungen"
      subtitle={`${records.length} Anmerkungen im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Anmerkungen suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Anmerkungen. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
