...
import { AnmerkungenViewDialog } from '@/components/dialogs/AnmerkungenViewDialog';
...
const ANMERKUNGEN_FIELDS = [
  { key: 'screenshot', label: 'Screenshot', type: 'file' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'prioritaet', label: 'Priorität', type: 'lookup/radio', options: [{ key: 'mittel', label: 'Mittel' }, { key: 'hoch', label: 'Hoch' }, { key: 'kritisch', label: 'Kritisch' }, { key: 'niedrig', label: 'Niedrig' }] },
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'geloest', label: 'Gelöst' }, { key: 'geschlossen', label: 'Geschlossen' }] },
];

const ENTITY_TABS = [
  { key: 'anmerkungen', label: 'Anmerkungen', pascal: 'Anmerkungen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('anmerkungen');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'anmerkungen': return (data as any).anmerkungen as Anmerkungen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'anmerkungen': return ANMERKUNGEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'anmerkungen': return {
        create: (fields: any) => LivingAppsService.createAnmerkungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAnmerkungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAnmerkungenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'anmerkungen' || dialogState?.entity === 'anmerkungen') && (
        <AnmerkungenDialog
          open={createEntity === 'anmerkungen' || dialogState?.entity === 'anmerkungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'anmerkungen' ? handleUpdate : (fields: any) => handleCreate('anmerkungen', fields)}
          defaultValues={dialogState?.entity === 'anmerkungen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Anmerkungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Anmerkungen']}
        />
      )}
      {viewState?.entity === 'anmerkungen' && (
        <AnmerkungenViewDialog
          open={viewState?.entity === 'anmerkungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'anmerkungen', record: r }); }}
        />
      )}
