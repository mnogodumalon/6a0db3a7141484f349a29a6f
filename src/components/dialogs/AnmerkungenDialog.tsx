...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Anmerkungen';
...
interface AnmerkungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Anmerkungen['fields']) => Promise<void>;
  defaultValues?: Anmerkungen['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function AnmerkungenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: AnmerkungenDialogProps) {
  const [fields, setFields] = useState<Partial<Anmerkungen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Anmerkungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'anmerkungen');
      await onSubmit(clean as Anmerkungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "beschreibung": string | null, // Beschreibung\n  "prioritaet": LookupValue | null, // Priorität (select one key: "mittel" | "hoch" | "kritisch" | "niedrig") mapping: mittel=Mittel, hoch=Hoch, kritisch=Kritisch, niedrig=Niedrig\n  "status": LookupValue | null, // Status (select one key: "offen" | "in_bearbeitung" | "geloest" | "geschlossen") mapping: offen=Offen, in_bearbeitung=In Bearbeitung, geloest=Gelöst, geschlossen=Geschlossen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Anmerkungen['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Anmerkungen bearbeiten' : 'Anmerkungen hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'screenshot': (
      <div key="screenshot" className="space-y-1.5">
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'geloest'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'geloest' ? undefined : 'geloest') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'geloest'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
  ...
  const FIELD_LABELS: Record<string, string> = {"screenshot": "Screenshot", "beschreibung": "Beschreibung", "prioritaet": "Priorität", "status": "Status"};
